...
public class TraVeGUI extends JPanel {
	...
	private JPanel buildTopInfoPanel() {
		...
		outer.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, true), new EmptyBorder(10, 10, 10, 10)));
		...
		// 3 hàng × 3 cột — mỗi field được ~1/3 chiều rộng, không bị cắt chữ
		JPanel infoGrid = new JPanel(new GridLayout(3, 3, 12, 8));
		...
		infoGrid.add(new JPanel()); // ô trống để fill đủ grid
	...
	private JPanel buildBottomDetailPanel() {
		...
		// ── Panel tài chính ──────────────────────────────────────────────
		JPanel pnlFinance = new JPanel(new BorderLayout(0, 10));
		pnlFinance.setBackground(Color.WHITE);
		pnlFinance.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, true), new EmptyBorder(12, 14, 12, 14)));
		...
		statPanelPhi = buildStatCard("Phí trả", lbStatPhi, new Color(254, 249, 235), new Color(251, 207, 100),
				new Color(160, 100, 0));
		...
		// ── Panel lý do ──────────────────────────────────────────────────
		JPanel pnlReason = new JPanel(new BorderLayout(0, 0));
		...
		txtLyDoKhac.setFont(FONT_14);
		txtLyDoKhac.setVisible(false);
		txtLyDoKhac.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1), new EmptyBorder(4, 10, 4, 10)));
		...
		// bỏ lbWarning khỏi UI — vẫn giữ field để calcFee() không NPE
		lbWarning = new JLabel(" ");
		lbWarning.setFont(FONT_14);
		lbWarning.setForeground(WARN_FG);
		lbWarning.setVisible(false); // ẩn hoàn toàn
		...
		pnlReason.add(fieldBox("Lý do trả vé (Khách hàng cung cấp)", lyDoWrapper), BorderLayout.CENTER);
	...
	private void fillInfo() {
		...
		String loaiVeChuan = "MOT_CHIEU".equalsIgnoreCase(s_data[3]) ? "Một chiều"
				: "KHU_HOI".equalsIgnoreCase(s_data[3]) ? "Khứ hồi" : s_data[3];
		tfLoai.setText(loaiVeChuan);
	...
	private void calcFee() {
		...
		try {
			donGia = (long) Double.parseDouble(s_data[8]);
		} catch (Exception ignored) {
		}
		...
		if (nhom) {
			if (gio >= 72) {
				phiPct = 20;
				hopLe = true;
				dieuKien = "Hợp lệ — phí 20% (vé nhóm)";
			} else if (gio >= 24) {
				phiPct = 30;
				hopLe = true;
				dieuKien = "Hợp lệ — phí 30% (vé nhóm)";
			} else {
				phiPct = -1;
				hopLe = false;
				dieuKien = "Quá hạn — dưới 24h (vé nhóm)";
			}
		} else {
			if (gio >= 48) {
				phiPct = 10;
				hopLe = true;
				dieuKien = "Hợp lệ — phí 10%";
			} else if (gio >= 12) {
				phiPct = 20;
				hopLe = true;
				dieuKien = "Hợp lệ — phí 20%";
			} else {
				phiPct = -1;
				hopLe = false;
				dieuKien = "Quá hạn — dưới 12h";
			}
		}
		...
		if (hopLe) {
			...
			lbWarning.setText("Vé hợp lệ. Nhấn 'Xác nhận trả vé' để tiến hành hoàn tiền.");
			...
			lbWarning.setText("Không đủ điều kiện hoàn tiền.");
			lbWarning.setForeground(WARN_FG);
			btnXacNhan.setEnabled(false);
		}
	}
	...
	private void clearInfo() {
		for (JTextField tf : new JTextField[] { tfMaVe, tfKhachHang, tfChuyen, tfGaDi, tfGaDen, tfNgayGio, tfLoai,
				tfMaGhe, tfViTri, tfGia })
			if (tf != null)
				tf.setText("—");
	}

	private void handleTiepTuc() {
		int choice = JOptionPane.showConfirmDialog(this, "Xác nhận trả vé " + s_maVe + "?", "Xác nhận",
		...
		try {
			tienHoanKhach = Long.parseLong(lbStatHoan.getText().replaceAll("[^0-9]", ""));
		} catch (Exception ignored) {
		}
		long phiHuyVe = 0;
		try {
			phiHuyVe = Long.parseLong(lbStatPhi.getText().replaceAll("[^0-9]", ""));
		} catch (Exception ignored) {
		}
		...
		DonDoiTraVeDAO donDAO = new DonDoiTraVeDAO();

		try (Connection conn = connect_DB.Connect_DB.getInstance().getConnection()) {
			conn.setAutoCommit(false);
			try {
				// 1. Tạo mã đơn
				String maDon = util.MaTuDong.taoMaDon(conn, java.time.LocalDate.now());

				// 2. Insert hóa đơn hoàn tiền — dùng HoaDonDAO
				boolean hdOk = hoaDonDAO.insertTrongTransaction(conn, maDon, currentNV,
						/* maKH lấy từ hóa đơn gốc — SELECT inline */
						layMaKHTuVe(conn, s_maVe), phiHuyVe, 0, "Hoàn tiền");
				if (!hdOk)
					throw new Exception("Không thể tạo hóa đơn hoàn tiền cho vé " + s_maVe);

				// 3. Cập nhật trạng thái vé — dùng VeDAO
				boolean veOk = veDAO.capNhatTrangThaiTrongTransaction(conn, s_maVe, "Đã hủy", maDon);
				if (!veOk)
					throw new Exception("Không tìm thấy vé " + s_maVe);

				// 4. Insert đơn trả vé — dùng DonDoiTraVeDAO
				donDAO.insertTrongTransaction(conn, maDon, phiHuyVe, tienHoanKhach, s_maVe);

				conn.commit();

				hienThiPopupThanhCong(maDon, tienHoanFinal, phiFinal);
			} catch (Exception ex) {
				conn.rollback();
				throw ex;
			}
	...
	/** Lấy maKH từ hóa đơn gốc của vé — helper nhỏ dùng trong transaction */
	private String layMaKHTuVe(Connection conn, String maVe) {
		try (PreparedStatement ps = conn.prepareStatement(
				"SELECT hd.maKH FROM HoaDon hd JOIN Ve v ON v.maHoaDon = hd.maHoaDon WHERE v.maVe = ?")) {
			ps.setString(1, maVe);
	...
	/** Tách phần popup thành công ra method riêng cho gọn */
	private void hienThiPopupThanhCong(String maDon, long tienHoan, long phi) {
		...
		javax.swing.JPanel box = new javax.swing.JPanel(new java.awt.BorderLayout(0, 12)) {
			@Override
			protected void paintComponent(java.awt.Graphics g) {
				java.awt.Graphics2D g2 = (java.awt.Graphics2D) g.create();
				g2.setRenderingHint(java.awt.RenderingHints.KEY_ANTIALIASING,
						java.awt.RenderingHints.VALUE_ANTIALIAS_ON);
				g2.setComposite(java.awt.AlphaComposite.getInstance(java.awt.AlphaComposite.SRC_OVER, alpha[0]));
		...
		javax.swing.JLabel lblMsg = new javax.swing.JLabel("<html><div style='text-align:center;'>"
				+ "<b style='font-size:15px;color:#1c396e;'>Trả vé thành công!</b><br>"
				+ "<span style='font-size:13px;color:#888;'>Đang xuất hóa đơn hoàn tiền...</span>" + "</div></html>",
		...
		javax.swing.Timer closeTimer = new javax.swing.Timer(2500, ev -> {
	        ...
	        dialog.dispose();
	        new Thread(() -> {
	            xuatHoaDonPDF(maDon, tienHoan, phi);
	            javax.swing.SwingUtilities.invokeLater(() -> appFrame.showCard("doi-tra"));
	        }).start();
	    });
	...
	private static long tinhGio(String s) {
		try {
			return ChronoUnit.HOURS.between(LocalDateTime.now(), LocalDateTime.parse(s, FMT));
		} catch (Exception e) {
			return -1;
		}
	}
	...
	private JPanel buildStatCard(String title, JLabel valueLb, Color bg, Color border, Color titleColor) {
		...
		JLabel lbTt = new JLabel(title);
		lbTt.setFont(FONT_14);
		lbTt.setForeground(titleColor);
		lbTt.setHorizontalAlignment(SwingConstants.CENTER);
		lbTt.setName("stat-title");
		valueLb.setHorizontalAlignment(SwingConstants.CENTER);
		p.add(lbTt, BorderLayout.NORTH);
		p.add(valueLb, BorderLayout.CENTER);
		return p;
	}
	...
	private JLabel statLabel(Color color, int style, int size) {
		...
		lb.setFont(new Font("Segoe UI", style, size));
	...
	private static String fmtTien(long a) {
			...
			protected void paintComponent(Graphics g) {
				...
				Color bg = !isEnabled() ? new Color(180, 190, 205)
						: getModel().isPressed() ? GuiTheme.NAVY_DARK
								: getModel().isRollover() ? GuiTheme.NAVY_HOVER : NAVY;
				g2.setColor(bg);
	...
	private void xuatHoaDonPDF(String maDon, long tienHoanKhach, long phiHuyVe) {
		try (Connection conn = connect_DB.Connect_DB.getConnection()) {
			...
			com.itextpdf.text.pdf.BaseFont bf = com.itextpdf.text.pdf.BaseFont.createFont("c:/windows/fonts/arial.ttf",
			...
			com.itextpdf.text.Font fontVAT = new com.itextpdf.text.Font(bf, 10, com.itextpdf.text.Font.ITALIC,
			...
			// ── Thông tin đơn vị ─────────────────────────────────────────────
			document.add(
					new com.itextpdf.text.Paragraph("Đơn vị bán hàng: CÔNG TY CỔ PHẦN VẬN TẢI ĐƯỜNG SẮT", fontBold));
			document.add(new com.itextpdf.text.Paragraph("Mã số thuế: 0100106264", fontNormal));
			document.add(
					new com.itextpdf.text.Paragraph("Địa chỉ: 113 Nguyễn Đình Thụ, Tuy Phước, Gia Lai", fontNormal));
			document.add(new com.itextpdf.text.Paragraph("Họ tên người bán: "
					+ (service.AuthService.getCurrentHoTen() != null ? service.AuthService.getCurrentHoTen() : "N/A")
					+ "          Mã NV: "
					+ (service.AuthService.getCurrentMaNV() != null ? service.AuthService.getCurrentMaNV() : "N/A"),
					fontNormal));
			document.add(new com.itextpdf.text.Paragraph(" ", fontNormal));
			...
			long tongTienVe = 0;
			try {
				tongTienVe = Long.parseLong(s_data[8].replaceAll("[^0-9]", ""));
			} catch (Exception ignored) {
			}
			...
			for (String h : new String[] { "STT", "Tên dịch vụ", "Loại vé", "Mã vé", "Chiều", "Giá vé", "ĐVT", "SL",
					"Phí trả", "Hoàn lại" }) {
			...
			e.printStackTrace();
			javax.swing.JOptionPane.showMessageDialog(this, "Lỗi xuất PDF: " + e.getMessage(), "Lỗi",
					javax.swing.JOptionPane.ERROR_MESSAGE);
		}
	}

	private void pdfAddCell(com.itextpdf.text.pdf.PdfPTable table, com.itextpdf.text.Font font, String text,
			int align) {
		com.itextpdf.text.pdf.PdfPCell cell = new com.itextpdf.text.pdf.PdfPCell(
