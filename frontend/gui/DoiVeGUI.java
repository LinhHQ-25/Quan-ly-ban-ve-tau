...
public class DoiVeGUI extends JPanel {
	...
	private JPanel buildOldTicketCard() {
		...
		card.setBorder(
				BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, true), new EmptyBorder(16, 20, 16, 20)));
	...
	private JPanel buildBottomBar() {
		...
		bar.setBorder(BorderFactory.createCompoundBorder(new MatteBorder(1, 0, 0, 0, BORDER),
		...
		btnTiepTuc.setIcon(GuiIcons.loadIcon(DoiTraGUI.class, "/Images/logoGoOn.png", 16, 16));
	...
	private void fillCurrentInfo() {
		...
		tfLoai.setText("MOT_CHIEU".equalsIgnoreCase(_rawLoai) ? "Một chiều"
				: "KHU_HOI".equalsIgnoreCase(_rawLoai) ? "Khứ hồi" : _rawLoai);
	...
	private void handleTiepTuc() {
		...
		if (gaDiMoi.equals(gaDenMoi)) {
			JOptionPane.showMessageDialog(this, "Ga đi và Ga đến không được trùng nhau!", "Lỗi",
	...
	private String[] loadDanhSachGa() {
		...
		// Tạo đối tượng DAO (nếu cần thiết có thể khai báo toàn cục trên class
		// DoiVeGUI)
	...
	private JDateChooser buildDateChooser(boolean enabled) {
		...
		dc.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, false), new EmptyBorder(2, 4, 2, 4)));
	...
	private JTextField readField() {
		...
		tf.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, false), new EmptyBorder(4, 8, 4, 8)));
	...
	private JComboBox<String> createComboBox(String[] items) {
		...
		cb.setForeground(GuiTheme.TEXT);
		cb.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER, 1, false), new EmptyBorder(2, 4, 2, 4)));
	...
	private JPanel newFieldBox(String label, JComponent comp) {
		...
		JLabel lb = new JLabel(label);
		lb.setFont(GuiTheme.font("Segoe UI", Font.BOLD, 13));
		lb.setForeground(GuiTheme.NAVY);
		lb.setAlignmentX(LEFT_ALIGNMENT);
	...
	private JButton makeNavyBtn(String text, int w, int h) {
		JButton btn = new JButton(text) {
			...
			protected void paintComponent(Graphics g) {
				...
				int totalWidth = textWidth + (icon != null ? iconWidth + iconTextGap : 0);
