...
public class DatVeGUI3 extends JPanel {
    ...
    public DatVeGUI3(DefaultTableModel modelFromGUI2, int secondsLeft, java.util.function.Consumer<Integer> onQuayLai, Runnable onHuyVe) {
    ...
    private Object[] layThongTinGheTuDB(String maGhe) {
        String loaiGhe = ""; double giaTien = 300000;
    ...
    private JPanel buildLeftPanel() {
        ...
        for (int i = 0; i < modelFromGUI2.getRowCount(); i++) {
            ...
            modelChiTiet.addRow(new Object[]{
                ...
                loaiDoiTuong, DF.format(giamGia) + " VNĐ", DF.format(donGia - giamGia) + " VNĐ"
            });
        }
    ...
    private JPanel buildRightPanel() {
        ...
        txtTienKhachDua.addKeyListener(new KeyAdapter() {
            public void keyReleased(KeyEvent e) {
                try {
                    ...
                    if (!raw.isEmpty()) txtTienKhachDua.setText(DF.format(Double.parseDouble(raw)));
        ...
        lblQR.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) { if (originalQRImageIcon != null) showZoomedQRDialog(); }
        });
        pnlQR.add(lblQR, BorderLayout.CENTER);
        pnlQR.addComponentListener(new ComponentAdapter() {
            @Override public void componentResized(ComponentEvent e) { if (originalQRImageIcon != null) scaleAndSetQR(); }
        });
    ...
    private JPanel buildBottomBar() {
        ...
        btnLuuTam.addActionListener(e -> {
            if (luuDuLieuVaoDatabase("Lưu tạm")) {
                ...
                stopAllTimers(); if (onQuayLai != null) onQuayLai.accept(secondsLeft);
            }
        });
        ...
        left.setBackground(Color.WHITE); left.add(btnQuayLai); left.add(btnHuyVe);
        JPanel right = new JPanel(new FlowLayout(FlowLayout.RIGHT,8,4));
        right.setBackground(Color.WHITE); right.add(lblCountdown); right.add(btnLuuTam); right.add(btnThanhToan);
    ...
    private void showHuyVePopup() {
        ...
        JLabel lblMsg = new JLabel("<html><div style='text-align:center;'><b style='font-size:14px;color:#1c396e;'>Xác nhận hủy vé?</b><br><br><span style='font-size:13px;color:#555;'>Tất cả thông tin đặt vé sẽ bị xóa.<br>Bạn sẽ được chuyển về trang tìm chuyến.</span></div></html>", SwingConstants.CENTER);
        JPanel topContent = new JPanel(new BorderLayout(0,10));
        topContent.setOpaque(false); topContent.add(lblIcon, BorderLayout.NORTH); topContent.add(lblMsg, BorderLayout.CENTER);
        ...
        glass.add(box); setupAndShowDialog(dialog, glass, ancestor);
    }
    ...
    private void showXacNhanThanhToanPopup() {
        ...
        box.setBorder(new EmptyBorder(28,32,24,32));
        String thuaHtml = tienKhach > 0 ? "<br><span style='font-size:14px;color:#555;'>Tiền thừa: <b style='color:#1a7a30;'>" + DF.format(Math.max(0, tienThua)) + " VNĐ</b></span>" : "";
        String thieu = tienKhach > 0 && tienThua < 0 ? "<br><span style='font-size:14px;color:#cc0000;'>⚠ Tiền khách đưa chưa đủ!</span>" : "";
        JLabel lblMsg = new JLabel("<html><div style='text-align:center;'><b style='font-size:15px;color:#1c396e;'>Xác nhận thanh toán tiền mặt</b><br><br><span style='font-size:14px;color:#555;'>Tổng thanh toán: <b style='color:#c82020;'>" + DF.format(tongThanhToan) + " VNĐ</b></span><br><span style='font-size:14px;color:#555;'>Khách đưa: <b>" + tienKhachStr + "</b></span>" + thuaHtml + thieu + "</div></html>", SwingConstants.CENTER);
        JPanel btnRow = new JPanel(new GridLayout(1,2,12,0));
        btnRow.setOpaque(false);
        JButton btnNo = makeOutlineBtn("Hủy", null);
        btnNo.addActionListener(ev -> dialog.dispose());
        JButton btnYes = makeNavyBtn("Xác nhận", null);
        btnYes.addActionListener(ev -> { dialog.dispose(); xuLyHoanTatThanhToan("Tiền mặt"); });
        btnRow.add(btnNo); btnRow.add(btnYes);
        box.add(lblMsg, BorderLayout.CENTER); box.add(btnRow, BorderLayout.SOUTH);
        glass.add(box); setupAndShowDialog(dialog, glass, ancestor);
    }
    ...
    private void showThanhCongPopup(Runnable onDone) {
        ...
        JLabel lblMsg = new JLabel("<html><div style='text-align:center;'><b style='font-size:15px;color:#1c396e;'>Thanh toán thành công!</b><br><span style='font-size:13px;color:#888;'>Đang xuất hóa đơn điện tử...</span></div></html>", SwingConstants.CENTER);
        box.add(lblMsg, BorderLayout.CENTER); glass.add(box);
        dialog.setContentPane(glass); dialog.setSize(ancestor.getSize()); dialog.setLocation(ancestor.getLocation());
        javax.swing.Timer animTimer = new javax.swing.Timer(16, null);
        animTimer.addActionListener(ev -> {
            frame[0]++; alpha[0] = Math.min(1f, frame[0] / 20f);
            glass.repaint(); box.repaint();
            if (frame[0] >= TOTAL_FRAMES) animTimer.stop();
        });
        animTimer.start();
        javax.swing.Timer closeTimer = new javax.swing.Timer(2500, ev -> { animTimer.stop(); dialog.dispose(); if (onDone != null) onDone.run(); });
        closeTimer.setRepeats(false); closeTimer.start();
    ...
    private void showZoomedQRDialog() {
        ...
        JPanel glass = new JPanel(new GridBagLayout()) {
            @Override protected void paintComponent(Graphics g) {
                g.setColor(new Color(10,20,50,150)); g.fillRect(0,0,getWidth(),getHeight());
            }
        };
        glass.setOpaque(false);
        ...
        JLabel lblTitle = new JLabel("Mã thanh toán hóa đơn"); lblTitle.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 15)); lblTitle.setForeground(NAVY);
        ...
        btnClose.setFont(new java.awt.Font("Segoe UI",java.awt.Font.BOLD,14)); btnClose.setForeground(new Color(200,40,40));
        btnClose.setContentAreaFilled(false); btnClose.setBorderPainted(false); btnClose.setFocusPainted(false); btnClose.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btnClose.setPreferredSize(new Dimension(28,28)); btnClose.addActionListener(e -> dialog.dispose());
        ...
        lblFoot.setFont(new java.awt.Font("Segoe UI",java.awt.Font.ITALIC,11)); lblFoot.setForeground(Color.GRAY);
        box.add(pnlHeader, BorderLayout.NORTH); box.add(lblBigQR, BorderLayout.CENTER); box.add(lblFoot, BorderLayout.SOUTH);
        glass.add(box); setupAndShowDialog(dialog, glass, ancestor);
    }
    ...
    private void startBankChecking() {
        ...
        bankCheckTimer = new javax.swing.Timer(5000, e -> {
            if (checkBankReceived()) {
                bankCheckTimer.stop(); xuLyHoanTatThanhToan("Chuyển khoản VietQR");
            }
        });
    ...
    private boolean checkBankReceived() {
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(code == 200 ? conn.getInputStream() : conn.getErrorStream(), StandardCharsets.UTF_8));
            StringBuilder sb = new StringBuilder(); String line;
            while ((line = br.readLine()) != null) sb.append(line);
            br.close();
            if (code != 200) return false;
            String jsonLower = sb.toString().toLowerCase();
            return jsonLower.contains(maHD.toLowerCase()) && jsonLower.contains(String.valueOf((long) tongThanhToan));
    ...
    private void xuLyHoanTatThanhToan(String hinhThuc) {
        ...
        if (luuDuLieuVaoDatabase(hinhThuc)) {
            showThanhCongPopup(() -> { taoHoaDonPDF(hinhThuc); quayVeTrangDau(); });
        }
    }
    ...
    private void quayVeTrangDau() {
        ...
        if (parent == null) { if (onHuyVe != null) { onHuyVe.run(); } return; }
        LayoutManager lm = parent.getLayout();
        if (lm instanceof CardLayout) {
            for (Component c : parent.getComponents()) { if (c instanceof DatVeGUI) ((DatVeGUI) c).resetForm(); }
            ((CardLayout) lm).show(parent, "dat-ve");
        } else {
            if (onHuyVe != null) { onHuyVe.run(); }
        }
    }
    ...
    private void tinhToanTaiChinh() {
        ...
        lblTongTien.setText("<html><font color='#505050'>Tổng tiền: </font><b>" + DF.format(tongGiaGoc) + " VNĐ</b></html>");
        lblTongKhuyenMai.setText("<html><font color='#505050'>Tổng KM: </font><b>" + DF.format(tongKM) + " VNĐ</b></html>");
        lblThanhToanConLai.setText("<html><font color='#505050'>Còn lại: </font><b><font color='#C82020'>" + DF.format(tongThanhToan) + " VNĐ</font></b></html>");
    ...
    private void tinhTienThua() {
        ...
        else if (thua < 0) { lblTienThua.setText("Thiếu: "+DF.format(Math.abs(thua))+" VNĐ"); lblTienThua.setForeground(Color.RED); }
        else { lblTienThua.setText(DF.format(thua)+" VNĐ"); lblTienThua.setForeground(new Color(0,140,0)); }
    }

    private void nhapKhuyenMai() {
        String ma = JOptionPane.showInputDialog(this, "Nhập mã khuyến mãi:");
        if (ma!=null && ma.trim().equalsIgnoreCase("SALE100")) { giamVoucher = 100000; } 
        else if (ma!=null && !ma.trim().isEmpty()) { giamVoucher = 0; } 
        else { giamVoucher = 0; }
        tinhToanTaiChinh();
    }
    ...
    private void toggleQRCode() {
        ...
        new SwingWorker<ImageIcon, Void>() {
            @Override protected ImageIcon doInBackground() throws Exception {
                ...
                String url = String.format("https://img.vietqr.io/image/970422-%s-compact2.png?amount=%s&addInfo=%s&accountName=%s", ACCOUNT_NO, (long) tongThanhToan, info, URLEncoder.encode(ACCOUNT_NAME, StandardCharsets.UTF_8));
    ...
    private JPanel buildPopupBox(int w, int h) {
        ...
        box.setOpaque(false); box.setPreferredSize(new Dimension(w, h)); return box;
    }

    private void setupAndShowDialog(JDialog dialog, JPanel glass, Window ancestor) {
        dialog.setContentPane(glass);
        dialog.getRootPane().registerKeyboardAction(ev -> dialog.dispose(), KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), JComponent.WHEN_IN_FOCUSED_WINDOW);
    ...
    private JPanel createDetailLabel(String title, String value) {
        ...
        p.add(l1, BorderLayout.WEST); p.add(l2, BorderLayout.EAST); return p;
    }
        ...
        JToggleButton b = new JToggleButton(text) {
            @Override protected void paintComponent(Graphics g) {
                ...
                if (isSelected()) {
                    ...
                    g2.setColor(NAVY); g2.setStroke(new BasicStroke(1.8f)); g2.drawRoundRect(1,1,getWidth()-3,getHeight()-3,4,4);
                } else {
                    g2.setColor(Color.WHITE); g2.fillRoundRect(0,0,getWidth(),getHeight(),4,4);
                    g2.setColor(BORDER_C); g2.setStroke(new BasicStroke(1f)); g2.drawRoundRect(0,0,getWidth()-1,getHeight()-1,4,4);
                }
                g2.dispose(); super.paintComponent(g);
            }
        };
        b.addChangeListener(e -> { b.setFont(b.isSelected() ? FONT_B14 : FONT_14); b.setForeground(b.isSelected() ? NAVY : new Color(80,80,80)); });
        b.setFont(FONT_14); b.setContentAreaFilled(false); b.setBorderPainted(false); b.setFocusPainted(false); b.setCursor(new Cursor(Cursor.HAND_CURSOR)); b.setBorder(new EmptyBorder(6,12,6,12));
        return b;
    }

    private Icon loadIcon(String path, int w, int h) {
        try { java.net.URL url = getClass().getResource(path); if (url!=null) return new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(w,h,Image.SCALE_SMOOTH)); } catch (Exception ignored) {} return null;
    }
    ...
    private JButton makeNavyBtn(String text, Icon icon) {
        ...
        b.setFont(FONT_B14); b.setForeground(Color.WHITE); b.setIconTextGap(8); b.setBorder(new EmptyBorder(6,18,6,18)); b.setContentAreaFilled(false); b.setBorderPainted(false); b.setFocusPainted(false); b.setCursor(new Cursor(Cursor.HAND_CURSOR)); return b;
    }
    ...
    private JButton makeOutlineBtn(String text, Icon icon) {
        JButton b = new JButton(text) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.fillRoundRect(0,0,getWidth(),getHeight(),6,6);
                g2.setColor(NAVY); g2.drawRoundRect(0,0,getWidth()-1,getHeight()-1,6,6); g2.dispose(); super.paintComponent(g);
            }
        };
    ...
    private JButton makeRedBtn(String text) {
        JButton b = new JButton(text) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(getModel().isPressed() ? new Color(160,20,20) : getModel().isRollover() ? new Color(200,40,40) : new Color(210,30,40));
                g2.fillRoundRect(0,0,getWidth(),getHeight(),8,8); g2.dispose(); super.paintComponent(g);
            }
        };
