...
public class DoiVeGUI0 extends JPanel {
    ...
    // ── Dữ liệu tĩnh truyền từ màn hình trước ────────────────────────────────
    private static String   s_maVeCu  = "";
    private static String[] s_dataCu  = new String[0];
    private static String   s_gaDi    = "", s_gaDen = "", s_loaiVe = "";
    private static String   s_ngayDi  = "", s_ngayVe = "";
    private static int      s_soLuong = 1;
    private static boolean  s_motChieu = true;

    public static void setTieuChiMoi(String maVeCu, String[] dataCu,
            String gaDi, String gaDen, String loaiVe,
            String ngayDi, String ngayVe, int soLuong) {
        s_maVeCu   = maVeCu;
    ...
    // Trạng thái hiện tại (dùng chung, giống DatVeGUI1)
    private int            trang     = 0;
    ...
    private JPanel pnlChuyenWrapper, pnlToaHeaderWrapper, hdrToa, stickyHeaderContainer;
    ...
    public DoiVeGUI0(AppFrame appFrame) {
        ...
        SwingUtilities.invokeLater(() -> {
            if (pnlAllToas != null) { pnlAllToas.revalidate(); pnlAllToas.repaint(); }
        });
    }

    public void refresh() {
        CHUYEN_FULL = chuyenTauDAO.loadChuyenFromDB(s_gaDi, s_gaDen, s_ngayDi);
        CHUYEN_FULL = filterChuyenDi(CHUYEN_FULL, s_ngayDi);
        ...
        if (cbLoaiToa  != null) cbLoaiToa.setSelectedIndex(0);

        lblChieuHeader.setText("Ngày " + s_ngayDi + " | " + s_gaDi + " → " + s_gaDen);
    ...
    // ── Lọc chuyến đi: giữ chuyến khởi hành sau now+15ph (chỉ khi là hôm nay) ─
    private String[][] filterChuyenDi(String[][] data, String ngayStr) {
        if (data == null || data.length == 0) return data;
        String today = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        if (ngayStr == null || !ngayStr.equals(today)) return data;
        LocalDateTime cutoff = LocalDateTime.now().plusMinutes(15);
        List<String[]> result = new ArrayList<>();
        for (String[] ch : data) {
            try {
                LocalDateTime tgDi = LocalDateTime.parse(ch[1], fmt);
                if (!tgDi.isBefore(cutoff)) result.add(ch);
            } catch (Exception ex) { result.add(ch); }
        }
    ...
    // ── Lọc theo khung giờ (giống DatVeGUI1) ────────────────────────────────
    private void applyFilter(String khungGio) {
        String[][] dataToFilter = dangXemChieuVe ? CHUYEN_FULL_VE : CHUYEN_FULL;
        String[][] result;

        if (dataToFilter == null || dataToFilter.length == 0
                || dataToFilter[0][0].equals("Không có chuyến")) {
            result = dataToFilter;
        } else {
            ...
            for (String[] ch : dataToFilter) {
                try {
                    ...
                    if (dangXemChieuVe && selectedArrivalDi != null
                            && tgDi.isBefore(selectedArrivalDi)) continue;
    ...
    // ── Lấy danh sách toa từ DB (giống DatVeGUI1) ───────────────────────────
    private String[] buildToaList(int ci) {
        String[][] filtered = dangXemChieuVe ? CHUYEN_FILTERED_VE : CHUYEN_FILTERED;
        if (filtered == null || filtered.length == 0
                || filtered[0][0].equals("Không có chuyến") || ci == -1)
            return new String[]{"--"};
        String tenTau = filtered[ci][0];
        List<String> toaList = new ArrayList<>();
        String sql = "SELECT t.maToaTau FROM ToaTau t JOIN Tau tau ON t.maTau = tau.maTau "
                   + "WHERE tau.tenTau = ? ORDER BY t.soToa";
        try (Connection con = Connect_DB.getInstance().getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    // ── gheDaDat: dùng VeDAO giống DatVeGUI1 ────────────────────────────────
    private Set<Integer> gheDaDat(String maChuyen, String maToa) {
        ...
        List<String> danhSachGhe = new VeDAO().layDanhSachMaGheDaDat(maChuyen);
        for (String fullMaGhe : danhSachGhe) {
    ...
    // ── getLoaiToa: nhất quán với DatVeGUI1 (≤4 cứng, ≤8 VIP, >8 mềm) ──────
    ...
    private void updateTopBarInfo() {
        if (pnlTopInfoContainer == null) return;
    ...
    private JPanel buildTopBar() {
        ...
        btnLamMoi.addActionListener(e -> {
            // Reload từ DB (giống DatVeGUI1 nút Làm mới)
            CHUYEN_FULL = chuyenTauDAO.loadChuyenFromDB(s_gaDi, s_gaDen, s_ngayDi);
            CHUYEN_FULL = filterChuyenDi(CHUYEN_FULL, s_ngayDi);
            if (CHUYEN_FULL == null || CHUYEN_FULL.length == 0)
                CHUYEN_FULL = new String[][]{ {"Không có chuyến","--:--","--:--",s_ngayDi,s_ngayDi,""} };
            ...
            if (!s_motChieu) {
                CHUYEN_FULL_VE = chuyenTauDAO.loadChuyenFromDB(s_gaDen, s_gaDi, s_ngayVe);
    ...
    private JPanel addInfoCell(String label, String value, boolean dimmed, int rightGap) {
        ...
        v.setBorder(new CompoundBorder(new LineBorder(new Color(200, 200, 200), 1),
    ...
    class SmoothScrollPanel extends JPanel implements Scrollable {
        ...
        public int getScrollableBlockIncrement(Rectangle vr, int o, int d) { return Math.max(vr.height * 2 / 3, 60); }
    ...
    private JPanel buildCenter() {
        ...
        topHeaderWrapper.add(new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0)) {{
            setOpaque(false); add(lblChieuHeader);
        }}, BorderLayout.WEST);
        ...
        Runnable startAnim = () -> {
            ...
            timer[0].addActionListener(tick -> {
                ...
                if (Math.abs(diff) < 0.5) {
                    currentPos[0] = targetPos[0]; timerRunning[0] = false;
                    timer[0].stop(); handleScrollEvents();
                } else {
    ...
    private void refreshChuyen() {
        ...
        if (filtered == null || filtered.length == 0 || filtered[0][0].equals("Không có chuyến")) {
            ...
            centerContent.add(new JLabel(loadAndScaleIcon("/Images/khongcochuyen.png", 60, 60)), gc);
            gc.gridy = 1; gc.insets = new Insets(5, 0, 0, 0);
            JLabel msg = new JLabel("<html><div style='text-align:center;'>Rất tiếc, hiện không có chuyến tàu nào phù hợp</div></html>");
            msg.setFont(new Font("Segoe UI", Font.BOLD, 15));
            centerContent.add(msg, gc);
    ...
    private JPanel buildCard(int ci) {
        ...
        Font fnorm = new Font("Segoe UI", Font.BOLD, 10);
        Font fdata = new Font("Segoe UI", Font.BOLD, 11);
        for (int i = 0; i < labels.length; i++) {
            JLabel l = new JLabel(labels[i]);
            l.setFont((i % 2 == 0) ? fnorm : fdata);
        ...
        JPanel card = new JPanel(new BorderLayout(0, 0)) {
            private boolean isHover = false;
            {
                addMouseListener(new MouseAdapter() {
                    ...
                    @Override public void mouseClicked(MouseEvent e) {
                        // Giống DatVeGUI1: khi chọn chuyến đi (khứ hồi) → filter lại chiều về
                        if (!s_motChieu) {
                            try {
                                if (!dangXemChieuVe) {
            ...
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.fillOval(w/4-8, trainH-14, 16, 16); g2.fillOval(3*w/4-8, trainH-14, 16, 16);
    ...
    private void refreshToaGhe() {
        ...
        String filterLoai = (String) cbLoaiToa.getSelectedItem();

        List<String> validToas = new ArrayList<>();
        for (String maToa : toaList) {
            String loai = getLoaiToa(maToa);
            if (!filterLoai.equals("Tất cả") && !loai.equals(filterLoai)) continue;
            validToas.add(maToa);
        }

        // Sắp xếp ưu tiên: nhất quán với DatVeGUI1 (VIP=18, Thường=28)
        validToas.sort((t1, t2) -> {
            String l1 = getLoaiToa(t1), l2 = getLoaiToa(t2);
            int max1 = l1.equals("VIP") ? 18 : 28;
            int max2 = l2.equals("VIP") ? 18 : 28;
            Set<Integer> b1 = gheDaDat(maChuyen, t1);
            Set<Integer> b2 = gheDaDat(maChuyen, t2);
            boolean adj1 = hasAdjacent(b1, s_soLuong, max1);
            boolean adj2 = hasAdjacent(b2, s_soLuong, max2);
            int g1 = getGroup(max1-b1.size(), adj1, (max1-b1.size())>=s_soLuong);
            int g2 = getGroup(max2-b2.size(), adj2, (max2-b2.size())>=s_soLuong);
            if (g1 != g2) return Integer.compare(g1, g2);
            return Integer.compare(
                    Integer.parseInt(t1.substring(1,3)),
                    Integer.parseInt(t2.substring(1,3)));
        });

        for (String maToa : validToas) {
            String loai = getLoaiToa(maToa);
            JPanel block = buildSingleToaBlock(maChuyen, maToa, loai);
            toaBlocks.add(block); toaMaToas.add(maToa); toaLoaiToas.add(loai);
    ...
    private JPanel buildSingleToaBlock(String maChuyen, String maToa, String loaiToa) {
        ...
        // maxSeats: VIP=18, Thường=28 (nhất quán DatVeGUI1)
        ...
        for (int i = 1; i <= maxSeats; i++) {
            ...
            if (!taken) {
                ...
                btn.addActionListener(ev -> {
                    // FIFO: giống DatVeGUI1 — bỏ ghế cũ nhất khi vượt soLuong
                    ...
                    if (!maToaFinal.equals(activeMaToa)) {
                        ...
                        if (idx >= 0) {
                            ...
                            String maCH = (chuyenIdx >= 0 && cur != null && chuyenIdx < cur.length) ? cur[chuyenIdx][5] : "";
                            Set<Integer> dadatInner = gheDaDat(maCH, maToaFinal);
                            int ms = loai.equals("VIP") ? 18 : 28;
                            int trong = 0;
                            for (int g = 1; g <= ms; g++) if (!dadatInner.contains(g)) trong++;
                            lblGheTrong.setText("Số ghế còn trống: " + trong + "/" + ms);
                            updateVisibleToa();
                        }
                    }
    ...
    private void updateVisibleToa() {
        ...
        int closestIndex = -1, minDiff = Integer.MAX_VALUE;
        ...
        // Cập nhật số ghế còn trống (maxSeats theo loaiToa)
    ...
    private void switchToChieuDi() {
        trangVe = trang; chuyenIdxVe = chuyenIdx;
        gheChonVe.clear(); gheChonVe.addAll(gheChon);
        dangXemChieuVe = false;
        trang = trangDi; chuyenIdx = chuyenIdxDi;
    ...
    private JPanel buildBotBar() {
        ...
        btnAction.addActionListener(e -> {
            if (gheChon.size() >= s_soLuong) {
                String maChuyenMoi = CHUYEN_FILTERED[chuyenIdx][5];
                String ngayMoi     = CHUYEN_FILTERED[chuyenIdx][1];
                String gheDiStr    = formatGheString(gheChon);
                String maToaMoi    = (activeMaToa != null) ? activeMaToa : extractToaFromGheStr(gheDiStr);

                DoiVeGUI1.setDonDoiKhuHoi(s_maVeCu, s_dataCu,
                        maChuyenMoi, ngayMoi, gheDiStr, maToaMoi, "—", "—");
                appFrame.showCard("doi-ve-step-2");
            } else {
                if (chuyenIdx < 0 || chuyenIdx >= CHUYEN_FILTERED.length) return;
                String maChuyen = CHUYEN_FILTERED[chuyenIdx][5];
                gheChon.clear();
                for (int i = 0; i < toaMaToas.size(); i++) {
                    ...
                    int ms = loaiToa.equals("VIP") ? 18 : 28;
                    Set<Integer> dadat = gheDaDat(maChuyen, maToa);
                    for (int g = 1; g <= ms; g++) {
                        if (gheChon.size() >= s_soLuong) break;
    ...
    private void updateGheDaChon() {
        if (lblGheDaChon != null)
            lblGheDaChon.setText("  Số vé đã chọn: " + gheChon.size() + "/" + s_soLuong);
    }
    ...
    private String formatGheString(Set<String> setGhe) {
        ...
        for (String seatId : setGhe) {
            ...
            sb.append(maToa).append(" - G").append(gNum);
        }
        return sb.toString();
    }
    ...
    private JButton makeOutlineBtn(String text, Icon icon) {
        JButton b = new JButton(text) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setColor(en ? (getModel().isPressed() ? new Color(198,215,242)
