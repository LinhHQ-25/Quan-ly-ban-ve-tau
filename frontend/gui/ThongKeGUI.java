...
public final class ThongKeGUI extends JPanel {
    ...
    private JPanel buildFilterPanel() {
        ...
        lbNgay.setFont(GuiTheme.font("Segoe UI", Font.BOLD, 13));
        dcNgay = new JDateChooser();
        dcNgay.setDateFormatString("dd/MM/yyyy");
        dcNgay.setPreferredSize(new Dimension(150, 28));

        JLabel lbCa = new JLabel("Ca làm việc:");
        lbCa.setFont(GuiTheme.font("Segoe UI", Font.BOLD, 13));
        cboCa = new JComboBox<>(new String[]{"Sáng", "Chiều"});
        cboCa.setPreferredSize(new Dimension(100, 28));
    ...
    private JPanel buildStatCard(String label, String value, Color accent, int idx) {
        ...
        card.setPreferredSize(new Dimension(210, 68)); card.setMaximumSize(new Dimension(210, 68));
    ...
    private JPanel buildTablePanel() {
        ...
        for (int i = 0; i < tblData.getColumnCount(); i++) tblData.getColumnModel().getColumn(i).setCellRenderer(zebraRenderer);
        ...
        pnlWrap.setOpaque(false); pnlWrap.add(spnScroll, BorderLayout.CENTER);
        return pnlWrap;
    }
    ...
    private JButton buildExportButton() {
        JButton btn = buildCustomButton("Xuất báo cáo kết ca", "/Images/DanhSach.png");
    ...
    private JButton buildCustomButton(String text, String iconPath) {
        ...
        btn.setFont(GuiTheme.font("Segoe UI", Font.BOLD, 13)); btn.setForeground(Color.WHITE);
        btn.setPreferredSize(new Dimension(180, 36)); btn.setContentAreaFilled(false);
...
class ChartPanel extends JPanel {
    ...
    public ChartPanel() {
        setPreferredSize(new Dimension(230, 320));
        ...
        addMouseListener(new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                ...
                if (old == selectedIdx) selectedIdx = -1; // Toggle off
    ...
    private int detectIndex(Point p) {
        ...
        if (dist >= INNER_R && dist <= OUTER_R) {
            ...
            // Map angle to start angles (sa = 90)
        ...
        for (int i = 0; i < 3; i++) {
            Rectangle r = new Rectangle(30, legendY + i * 24, 180, 20);
    ...
    @Override protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i == 2 && total != 0) arc = 360 - (int)Math.round(360.0 * data[0]/total) - (int)Math.round(360.0 * data[1]/total); // Ensure full 360
            Color c = COLORS[i];
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Color c = COLORS[i];
            if (selectedIdx != -1 && selectedIdx != i) c = new Color(c.getRed(), c.getGreen(), c.getBlue(), 60);
            g2.setColor(c); g2.fillRoundRect(30, legendY + i * 24, 14, 14, 4, 4);
            g2.setColor(new Color(50, 55, 75)); g2.setFont(GuiTheme.font("Segoe UI", Font.BOLD, 12));
            g2.drawString(LABELS[i], 55, legendY + i * 24 + 12);
            g2.setColor(new Color(120, 125, 145)); g2.setFont(GuiTheme.font("Segoe UI", Font.PLAIN, 12));
            String pctStr = String.format("%.1f%%", pct) + " (" + data[i] + ")";
            g2.drawString(pctStr, 140, legendY + i * 24 + 12);
            
            if (selectedIdx == i) {
                g2.setColor(COLORS[i]);
                g2.setStroke(new BasicStroke(1));
                g2.drawRoundRect(25, legendY + i * 24 - 4, 185, 24, 6, 6);
            }
        }
        g2.dispose();
    }
