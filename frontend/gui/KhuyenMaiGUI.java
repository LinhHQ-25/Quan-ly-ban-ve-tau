...
public class KhuyenMaiGUI extends JPanel {
    ...
    public KhuyenMaiGUI() {
        ...
        // Wrap center + nút thêm góc phải dưới
    ...
    // =========================================================
    // TOP BAR — tiêu đề + filter tags + search (không có nút Thêm)
    // =========================================================
    private JPanel buildTopBar() {
        ...
        // Hàng filter tags + search
        JPanel row = new JPanel(new BorderLayout(0, 0));
        row.setOpaque(false);
        row.setAlignmentX(LEFT_ALIGNMENT);
        row.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));

        // Filter tags bên trái
        ...
        // Search bên phải — bo góc 20 như login
    ...
    // =========================================================
    // BOTTOM BAR — nút Thêm góc phải dưới, màu NAVY như login
    // =========================================================
    ...
    private JPanel buildTableArea() {
        ...
        table.getColumnModel().getColumn(7).setCellRenderer((tbl, val, sel, foc, row, col) -> {
            ...
            lbl.setOpaque(true);
            lbl.setBackground(tbl.isRowSelected(row)
                    ? new Color(219, 234, 254)
                    : (row % 2 == 0 ? Color.WHITE : new Color(248, 250, 252)));
            return lbl;
        });

        // Độ rộng cột — tổng ~870px, cột ⋮ luôn hiện
    ...
    private void loadData() {
        ...
        for (KhuyenMai km : dao.selectAll()) {
            ...
            String bdStr   = km.getThoiGianBatDau()  != null
                    ? sdf.format(java.sql.Timestamp.valueOf(km.getThoiGianBatDau()))  : "";
            String ktStr   = km.getThoiGianKetThuc() != null
                    ? sdf.format(java.sql.Timestamp.valueOf(km.getThoiGianKetThuc())) : "";
            String ttStr   = km.getTrangThai() ? "Đang áp dụng" : "Ngừng áp dụng";

            allRows.add(new Object[]{
                km.getMaKhuyenMai(), km.getTenKhuyenMai(),
                tileStr, loaiKH, bdStr, ktStr, ttStr, ""
            });
        }
        applySearch();
    }
    ...
    private JButton makeFilterTag(String label, String status, Color bg, Color fg) {
        JButton b = new JButton(label) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setColor(active
                        ? (status.equals("ALL") ? TAG_ALL_FG
                           : status.equals("ACTIVE") ? TAG_ACT_FG : TAG_OFF_FG)
                        : bg);
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 20, 20);
    ...
    static class KhuyenMaiFormDialog extends JDialog {
        ...
        private JPanel buildForm() {
            ...
            tfTenKM      = inputField();
            tfTiLe       = inputField();
            tfTiLe.setToolTipText("Ví dụ: 0.2 (tương đương 20%)");
            tfThoiGianBD = inputField();
            tfThoiGianBD.setToolTipText("Định dạng: dd/MM/yyyy");
            tfThoiGianKT = inputField();
            tfThoiGianKT.setToolTipText("Định dạng: dd/MM/yyyy");
        ...
        private void loadKhuyenMai(String maKM) {
            ...
            tfThoiGianBD.setText(km.getThoiGianBatDau()  != null
                    ? km.getThoiGianBatDau().toLocalDate().format(FMT)  : "");
            tfThoiGianKT.setText(km.getThoiGianKetThuc() != null
                    ? km.getThoiGianKetThuc().toLocalDate().format(FMT) : "");
        ...
        private void saveKhuyenMai() {
            ...
            if (maKM.isEmpty() || tenKM.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập Mã KM và Tên KM.",
            ...
            try {
                String s = tfThoiGianBD.getText().trim();
                if (!s.isEmpty()) bd = LocalDate.parse(s, FMT).atStartOfDay();
            } catch (Exception ignored) {}
            try {
                String s = tfThoiGianKT.getText().trim();
                if (!s.isEmpty()) kt = LocalDate.parse(s, FMT).atStartOfDay();
            } catch (Exception ignored) {}
        ...
        // Nút dùng chung cho dialog — navy (filled) hoặc outline
