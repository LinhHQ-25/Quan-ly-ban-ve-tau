...
public class DoiVeGUI2 extends JPanel {
    ...
    private JPanel buildLeftPanel() {
        ...
        String[] cols = {"STT","Mã vé","Chuyến Đổi Sang","Ghế mới","Ngày/Giờ KH","Lệ phí đổi","Chênh lệch","Tổng thu"};
    ...
    private JPanel buildRightPanel() {
        ...
        String[] quickCash = {"30,000","50,000","100,000","200,000","500,000","1,000,000","1,500,000","2,000,000"};
        ...
        lblQR.setToolTipText("Click vào để phóng to mã QR");
    ...
    private JPanel buildBottomBar() {
        ...
        bar.setBorder(BorderFactory.createCompoundBorder(
                new MatteBorder(1, 0, 0, 0, BORDER_C), new EmptyBorder(15, 0, 0, 0)));
        ...
        JPanel pnlLeftBtn  = new JPanel(new FlowLayout(FlowLayout.LEFT,  0, 0)); pnlLeftBtn.setOpaque(false);  pnlLeftBtn.add(btnQuayLai);
        JPanel pnlRightBtns = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0)); pnlRightBtns.setOpaque(false);
        pnlRightBtns.add(lblCountdown); pnlRightBtns.add(btnThanhToan);
    ...
    private void tinhToanTaiChinh() {
        ...
        lblTongTien.setText("<html><font color='#505050'>Lệ phí cố định: </font><font color='black'><b>" + DF.format(lePhi) + "</b></font></html>");
        lblTongKhuyenMai.setText("<html><font color='#505050'>Chênh lệch vé: </font><font color='black'><b>" + (chenhLech > 0 ? "+" : "") + DF.format(chenhLech) + "</b></font></html>");
    ...
    private void tinhTienThua() {
        if (s_tongThu <= 0) return;
        if (txtTienKhachDua == null || lblTienThua == null) return;
        double tienKhach = parseMoney(txtTienKhachDua.getText());
        double tienThua  = tienKhach - s_tongThu;

        if (tienKhach == 0) {
            lblTienThua.setText("0 VNĐ"); lblTienThua.setForeground(Color.BLACK);
        } else if (tienThua < 0) {
            lblTienThua.setText("Còn thiếu: " + DF.format(Math.abs(tienThua))); lblTienThua.setForeground(Color.RED);
        } else {
            lblTienThua.setText(DF.format(tienThua)); lblTienThua.setForeground(Color.BLACK);
        }
    }
    ...
    private void xuLyHoanTatThanhToan(String hinhThuc) {
        ...
        // 1. Tạo maHoaDon mới cho đơn đổi vé
        String maHoaDonMoi;
        try (Connection conn = Connect_DB.getInstance().getConnection()) {
            maHoaDonMoi = MaTuDong.taoMaHoaDon(conn, LocalDate.now());
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi tạo mã hóa đơn: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // 2. Cập nhật Ve: đổi maChuyenTau và maGhe
        String sqlUpdateVe =
                "UPDATE Ve SET maChuyenTau = (" +
                        "  SELECT TOP 1 dt.maChuyenTau FROM ChiTietChuyenTau dt " +
                        ...
                        "  AND gDi.tenGa = ? AND gDen.tenGa = ?" +
                        "), maGhe = ?, maHoaDon = ? WHERE maVe = ?";

        // 3. Tạo bản ghi HoaDon mới cho đơn đổi
        String sqlInsertHD =
        ...
        try (Connection conn = Connect_DB.getInstance().getConnection()) {
            ...
            try {
                // Cập nhật vé
                try (PreparedStatement psVe = conn.prepareStatement(sqlUpdateVe)) {
                    psVe.setString(1, s_chuyenMoi);
                    psVe.setString(2, s_ngayMoi);
                    psVe.setString(3, safe(s_dataCu, 1));
                    psVe.setString(4, safe(s_dataCu, 2));
                    psVe.setString(5, s_gheMoiDB);
                    psVe.setString(6, maHoaDonMoi);
                    psVe.setString(7, s_maVe);
                    int rows = psVe.executeUpdate();
                    if (rows == 0) throw new Exception("Không tìm thấy vé hoặc chuyến tàu!");
                }

                // Tạo hóa đơn mới
                try (PreparedStatement psHD = conn.prepareStatement(sqlInsertHD)) {
                    psHD.setString(1, maHoaDonMoi);
                    psHD.setLong  (2, s_tongThu);
                    psHD.setLong  (3, s_tongThu > 0 ? s_tongThu : 0);
                    psHD.setString(4, hinhThuc);
                    psHD.setString(5, s_maVe);
                    psHD.executeUpdate();
                }
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Lỗi xử lý: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
        ...
        if (btnChuyenKhoan.isSelected() && s_tongThu > 0) {
            ...
            SwingWorker<ImageIcon, Void> worker = new SwingWorker<>() {
                @Override protected ImageIcon doInBackground() throws Exception {
                    ...
                    String amount    = String.valueOf(s_tongThu);
                ...
                @Override protected void done() {
                    ...
                    } catch (Exception e) { lblQR.setIcon(null); lblQR.setText("Lỗi load mã QR!"); }
                }
            };
            worker.execute();
        }
    ...
    private void showZoomedQRDialog() {
        ...
        JPanel pnlGlass = new JPanel(new GridBagLayout()) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setColor(new Color(0, 0, 0, 150)); g2.fillRect(0, 0, getWidth(), getHeight()); g2.dispose();
            }
        };
        ...
        pnlBox.setOpaque(false); pnlBox.setPreferredSize(new Dimension(420, 460));
        ...
        btnClose.setCursor(new Cursor(Cursor.HAND_CURSOR)); btnClose.setPreferredSize(new Dimension(28, 28));
        ...
        JLabel lblFoot  = new JLabel("Mở ứng dụng Ngân hàng / Ví điện tử quét để hoàn tất", SwingConstants.CENTER);
    ...
    private void startBankChecking() {
        ...
        bankCheckTimer = new javax.swing.Timer(5000, new ActionListener() {
            ...
            @Override public void actionPerformed(ActionEvent e) {
                ...
                if (checkCount >= 3) {
                    bankCheckTimer.stop();
                    for (Window w : Window.getWindows()) if (w instanceof JDialog && w.isVisible()) w.dispose();
    ...
    private JToggleButton createToggleBtn(String text) {
        JToggleButton b = new JToggleButton(text) {
            @Override protected void paintComponent(Graphics g) {
                ...
                if (isSelected()) {
                    ...
                    g2.setColor(isEnabled() ? Color.WHITE : new Color(240, 240, 240));
                    g2.fillRoundRect(0, 0, getWidth(), getHeight(), 4, 4);
                    g2.setColor(BORDER_C); g2.setStroke(new BasicStroke(1.0f));
        ...
        b.addChangeListener(e -> {
            if (b.isSelected()) { b.setFont(FONT_B14); b.setForeground(NAVY); }
            else { b.setFont(FONT_14); b.setForeground(new Color(80, 80, 80)); }
        });
        b.setFont(FONT_14); b.setContentAreaFilled(false); b.setBorderPainted(false);
        b.setFocusPainted(false); b.setCursor(new Cursor(Cursor.HAND_CURSOR));
        b.setBorder(new EmptyBorder(6, 12, 6, 12)); return b;
    }
    ...
    private JButton makeNavyBtn(String text, Icon icon) {
        ...
        if (icon != null) b.setIcon(icon); b.setFont(FONT_B14); b.setForeground(Color.WHITE);
        b.setIconTextGap(8); b.setBorder(new EmptyBorder(6, 18, 6, 18));
    ...
    private Icon loadIcon(String path, int w, int h) {
        try { URL url = getClass().getResource(path); if (url != null) return new ImageIcon(new ImageIcon(url).getImage().getScaledInstance(w, h, Image.SCALE_SMOOTH)); }
